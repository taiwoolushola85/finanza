<?php
$branch = @$_POST['branch']; // search by branch
?>
...
$fp = fopen('../data/staff_by_branch_list.json', 'w'); 
...
<tbody>
<?php
$url = '../data/staff_by_branch_list.json';
...
<?php 
include '../config/db.php';
$result = mysqli_query($con, "SELECT COUNT(*) FROM users WHERE Branch_id = '$branch'");
$row = mysqli_fetch_array($result);
$total = $row[0];
echo $total;
mysqli_close($con);
...
include '../config/user_session.php';
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT id, Staff_ID, Name, Branch, User_Group, Email, Phone, Country, Location, Status FROM users WHERE Branch_id = '$branch' ORDER BY Name ASC") or die("Bad Query.");
