...
$stmt = $con->prepare("SELECT id, Firstname, Middlename, Amount, Status, Date_Paid, Time_Paid FROM history WHERE Team_Leader = ? AND Date_Paid = ? ORDER BY id DESC LIMIT 20");
...
file_put_contents('../data/recent_team_transaction.json', json_encode($results));
