...
<div class="col-sm-8">
<b style="margin-left: 8px;">Staff ID:</b> <span><?php echo $stid; ?></span>
</div>
<div class="col-sm-4" >
<?php 
if($stid == 'NA'){
?>
<a href="#" style="font-size:11px;" data-bs-toggle="modal" data-bs-target="#modal6">+ Update</a>
...
<div class="col-sm-6">
<b style="margin-left: 8px;">User ID:</b> <span><?php echo $usid; ?></span>
</div>
</div>
<div class="row">
<div class="col-sm-12">
...
$result = mysqli_query($con, "SELECT count(*) FROM repayments WHERE User ='$User' AND Status != 'Cancelled'");
...
$d = date('Y-m-d');
include '../config/db.php';
$result = mysqli_query($con, "SELECT SUM(Loan_Amount) FROM repayments WHERE User ='$User' AND Status != 'Cancelled'");
$row = mysqli_fetch_array($result);
$total = $row[0];
...
$d = date('Y-m-d');
include '../config/db.php';
$result = mysqli_query($con, "SELECT SUM(Total_Bal) FROM repayments WHERE User ='$User' AND Status = 'Active'");
$row = mysqli_fetch_array($result);
$total = $row[0];
echo number_format($total,2);
?>
...
<a class="nav-item nav-link active" id="nav2-home-tab" data-bs-toggle="tab" href="#nav2-home" aria-selected="true" role="tab"><i class="fa fa-star"></i> Target</a>
<a class="nav-item nav-link" id="nav2-profile-tab" data-bs-toggle="tab" href="#nav2-profile" aria-selected="false" tabindex="-1" role="tab"><i class="fa fa-edit"></i> Password</a>
...
<br>
<br>
<b>Set Your Monthly Target</b>
<div class="col-sm-3">
<form action="" method="POST" enctype="multipart/form-data" id="uploadTarget">
<input type="text" name="id" value="<?php echo $usid; ?>" required hidden >
<input type="text" class="form-control" name="target" placeholder="Enter your sale target" value="<?php echo $target; ?>" required="required">
<br>
<button type="submit" class="btn btn-outline-info btn-sm w-100" id="up"  onclick="data()">Update Target</button>
</form>
</div>
