...
include '../config/db.php';
$username = $_GET['id'];
$sql = "SELECT COUNT(*) AS overs FROM history WHERE User = '$username' AND Status = 'Waiting For Approval' AND Post_Method = 'Basic Posting'";
...
include '../config/db.php';
$username = $_GET['id'];
$sql = "SELECT COALESCE(SUM(Amount), 0) AS overs FROM history WHERE User = '$username' AND Status ='Waiting For Approval' AND Post_Method = 'Basic Posting'";
$result=mysqli_query($con,$sql);
$data=mysqli_fetch_assoc($result);
$over = $data['overs'];
...
include '../config/db.php';
$username = $_GET['id'];
$sql = "SELECT COALESCE(SUM(Savings), 0) AS overs FROM save WHERE User = '$username' AND Status ='Waiting For Approval' AND Posting_Method = 'Basic Posting'";
$result=mysqli_query($con,$sql);
$data=mysqli_fetch_assoc($result);
$overs = $data['overs'];
echo "Total Savings: ".number_format($overs,2);
mysqli_close($con);
?>
</div>
<div class="col-sm-3">
<?php 
echo "Total Posting: ".number_format($over + $overs,2);
?>
</div>
...
$d = date('Y-m-d');
$result = mysqli_query($con, "SELECT id, Firstname, Middlename, Lastname, Unions, Loan_Type, Expected_Amount, Amount, Savings, Expected_Amount, 
Payment_Method, Post_Method, Location, Officer_Name, Date_Paid, Time_Paid FROM history WHERE User = '$username' AND Status = 'Waiting For Approval' 
AND Post_Method = 'Basic Posting' ORDER BY Time_Paid ASC") or die("Bad Query.");
...
<td class="sort border-top" style="text-transform:uppercase"><?php echo $member->Loan_Type?></td>
