...
if ($savingsType === 'Flexi') {
    // FLEXI LOGIC
    $whereClause = "Status != ? AND Team_Leader = '$User'";
    $params = ['Closed'];
    $paramTypes = 's';
...
$(document).ready(function() {
    // 1. .off('click') is the cure. It removes old listeners before adding the new one.
    // Use a namespace like '.savingsProfile' to avoid interfering with other scripts.
    $(document).off('click.savingsProfile').on('click.savingsProfile', '.invk, .invks', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation(); // Prevents other scripts from firing on the same click
        ...
        // 2. Select target URL
        const targetUrl = $this.hasClass('invk') ? 'client_flexi_saving_page.php' : 'client_team_saving_page.php';

        // 3. Open modal immediately
        $updateModal.modal('show');

        // 4. Loading UI (with min-height to prevent modal collapsing/blinking)
        $profile.html(`
        ...
        $.ajax({
            ...
            success: function(data) {
                // 6. Stop any active animations and swap content
                $profile.stop(true, true).hide().html(data).fadeIn(200);
            },
            error: function() {
                $profile.html('<div class="alert alert-danger m-3 text-center">Connection error. Please try again.</div>');
            }
        });
    });
});
